...
const ASSIGNMENT_TYPES = [
  { value: 'written',        label: 'تحريري',      icon: '📝' },
  { value: 'oral',           label: 'شفهي',        icon: '🗣️' },
  { value: 'practical',      label: 'عملي',        icon: '🔬' },
  { value: 'project',        label: 'مشروع',       icon: '📁' },
  { value: 'quiz',           label: 'اختبار',      icon: '✅' },
  { value: 'notebook_photo', label: 'صورة دفتر',   icon: '📸' },
]
...
export function CreateAssignmentPage() {
  ...
  function validate(): boolean {
    ...
    if (!title.trim()) errs.title = 'عنوان الواجب مطلوب'
    if (!dueDate)       errs.dueDate = 'تاريخ الاستحقاق مطلوب'
    const g = parseFloat(maxGrade)
    if (isNaN(g) || g <= 0) errs.maxGrade = 'الدرجة يجب أن تكون أكبر من صفر'
    setErrors(errs)
  ...
  return (
    <PageWrapper>
      <AppBar title="واجب جديد" onBack={() => navigate(-1)} />

      <form onSubmit={handleSubmit} className="p-4 space-y-6">

        <ArabicInput
          label="عنوان الواجب"
          placeholder="اكتب عنوان الواجب هنا..."
          value={title}
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 font-arabic text-right mb-3">
            اختر النوع
          </label>
          <TypeSelector
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          />
          <div>
            <label className="block text-sm font-medium text-gray-700 font-arabic text-right mb-1">
              تاريخ الاستحقاق
            </label>
            <input
              type="date"
              ...
              className={`w-full px-3 py-3 rounded-xl border text-right font-arabic focus:outline-none focus:ring-2 focus:ring-teal/30 focus:border-teal ${
                errors.dueDate ? 'border-red-400 bg-red-50' : 'border-gray-200'
              }`}
            />
            {errors.dueDate && <p className="text-red-600 text-xs font-arabic mt-1">{errors.dueDate}</p>}
          </div>
        </div>

        <div>
          <label className="block text-sm font-medium text-gray-700 font-arabic text-right mb-1">
            الفصل / تعليمات (اختياري)
          </label>
          <textarea
            value={description}
        ...
        <div className="flex items-center justify-between bg-[#25D366]/10 rounded-xl px-4 py-3">
          <div>
            <p className="font-medium font-arabic text-gray-900 text-sm">إرسال WhatsApp للأولياء</p>
            <p className="text-xs text-gray-500 font-arabic">يُرسل تنبيه فور النشر</p>
          </div>
          <button
            type="button"
        ...
        >
          {submitting ? 'جاري النشر...' : 'نشر الواجب ✓'}
        </button>
      </form>
    </PageWrapper>
  )
