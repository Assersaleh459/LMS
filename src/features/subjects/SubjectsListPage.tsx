    ...
    <PageWrapper>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-2">
        {loading ? (
          ...
          <p className="text-center text-gray-400 font-arabic text-sm py-20">لا توجد مواد مسجلة</p>
        ) : (
          subjects.map(sub => (
            <button
              key={sub.id}
            ...
            >
              <div className="w-12 h-12 rounded-2xl bg-navy/10 flex items-center justify-center flex-shrink-0">
                <span className="text-navy font-bold font-arabic text-xs leading-tight text-center">{sub.name_ar.slice(0, 2)}</span>
              </div>
              <div className="flex-1 min-w-0">
                <p className="font-bold font-arabic text-gray-900 text-sm truncate">{sub.name_ar}</p>
                <p className="text-gray-400 font-arabic text-xs mt-0.5">
                  {STAGE_LABELS[sub.stage] ?? sub.stage} — الصف {sub.grade_year}
                </p>
              </div>
