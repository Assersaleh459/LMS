...
export function AttendanceCalendar({ records }: AttendanceCalendarProps) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 mx-4 mb-4">
      <h3 className="font-bold font-arabic text-gray-900 mb-3 text-sm">
        {ARABIC_MONTHS[month]} {toArabicNumerals(year)}
      </h3>

      <div className="grid grid-cols-7 gap-1 text-center mb-2">
        {ARABIC_DAYS_SHORT.map(d => (
          <span key={d} className="text-xs text-gray-400 font-arabic">{d}</span>
        ))}
      </div>
      ...
      <div className="flex items-center gap-4 mt-3 justify-center">
        {[
          ...
          { color: 'bg-yellow-400', label: 'متأخر' },
        ].map(({ color, label }) => (
          <div key={label} className="flex items-center gap-1">
            <div className={`w-3 h-3 rounded-full ${color}`} />
            <span className="text-xs text-gray-500 font-arabic">{label}</span>
          </div>
        ))}
      </div>
    </div>
